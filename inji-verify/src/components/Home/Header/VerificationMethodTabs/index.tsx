...
const Tab = ({active, label, disabled, onClick}: {active: boolean, label: string, disabled?: boolean, onClick?: () => void}) => {
    ...
    const disabledTab = "cursor-not-allowed text-gray-600 bg-gray-200 align-s";
...
function VerificationMethodTabs(props: any) {
    ...
    function selectVerificationMethod(method: VerificationMethod) {dispatch(selectMethod({method}))}
    return (
            ...
            <div className="flex flex-row items-center mx-auto justify-center">
                <div className="w-full">
                    <div className="flex space-x-0.5 border-b border-gray-200 font-bold">
                        <Tab active={method === "UPLOAD"} label="Upload QR Code" onClick={() => selectVerificationMethod("UPLOAD")}/>
                        <Tab active={method === "SCAN"} label="Scan the QR Code" onClick={() => selectVerificationMethod("SCAN")}/>
