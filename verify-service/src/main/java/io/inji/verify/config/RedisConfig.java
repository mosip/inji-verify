        ...
        return (builder) -> {
            ...
            if (redisConfigProperties.isAuthRequestCacheEnabled()) {
                builder.withCacheConfiguration("authorizationRequestCache",
                        RedisCacheConfiguration.defaultCacheConfig().entryTtl(java.time.Duration.ofHours(redisConfigProperties.getAuthRequestCacheTtlHours())));
            }
            if (redisConfigProperties.isPresentationDefinitionCacheEnabled()) {
                builder.withCacheConfiguration("presentationDefinitionCache",
                        RedisCacheConfiguration.defaultCacheConfig().entryTtl(java.time.Duration.ofHours(redisConfigProperties.getPresentationDefinitionCacheTtlHours())));
            }
        };
    }
