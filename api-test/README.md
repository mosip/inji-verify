...
2. Run the automation test suite JAR file:
   ```
   java -jar -Dmodules=injiverify -Denv.user=api-internal.<env_name> -Denv.endpoint=<base_env> -Denv.testLevel=smokeAndRegression -jar apitest-injiverify-0.11.0-jar-with-dependencies.jar
   ```
...
- **jar**: Specify the name of the JAR file to execute. The version will change according to the development code version. For example, the current version may look like `apitest-injiverify-0.10.0-jar-with-dependencies.jar`.
